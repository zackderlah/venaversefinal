...
export default function GamesPage() {
  ...
  const [allGameReviews, setAllGameReviews] = useState<Review[]>([]);
  const [reviewsLoading, setReviewsLoading] = useState(true);
  const [viewMode, setViewMode] = useState<'all' | 'my'>('my');

  useEffect(() => {
    fetch('/api/reviews/games')
      .then(res => res.json())
      .then((data: Review[]) => {
        setAllGameReviews(data);
        setReviewsLoading(false);
      });
  }, []);

  const filteredByViewMode = viewMode === 'my' && currentUser
    ? allGameReviews.filter(review => review.userId === Number(currentUser.id))
    : allGameReviews;
    ...
    <div>
      <section>
        ...
        <p className="text-gray-600 dark:text-gray-300 mb-6 text-lg lowercase">
          {viewMode === 'my'
            ? (currentUser ? `a collection of game reviews written by you.` : `please log in to see your game reviews.`)
            : `a collection of thoughts and ratings for games.`}
        </p>
        <div className="space-y-8">
          <SearchBar value={search} onChange={setSearch} placeholder="search games by title..." />
          ...
          {sortedAndFilteredReviews.length > 0 ? (
            ...
            <p className="text-center text-gray-500 lowercase">
              {viewMode === 'my' && !currentUser
                ...
                : (viewMode === 'my' && currentUser ? 'you haven\'t written any game reviews yet.' : 'no game reviews found.')}
            </p>
          )}
        </div>
      )}
